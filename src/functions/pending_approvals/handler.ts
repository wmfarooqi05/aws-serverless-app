...
import { IPendingApprovalModel } from "@models/PendingApproval";
...
const getPendingApprovalByIdHandler = async (event) => {
  try {
    const { requestId } = event.params;
    const resp = await container
      .resolve(PendingApprovalService)
      .getPendingApprovalById(event.employee, requestId);
    return formatJSONResponse(resp, 200);
  } catch (e) {
    return formatErrorResponse(e);
  }
...
export const getPendingApprovalById = checkRolePermission(
  getPendingApprovalByIdHandler,
